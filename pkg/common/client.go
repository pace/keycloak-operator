...
func (c *Client) create(obj T, resourcePath, resourceName string) (string, error) {
	...
	if res.StatusCode != 201 && res.StatusCode != 204 {
		...
		if err != nil {
			logrus.Infof("Failed processing body with: %+v", err)
		}

		return "", errors.Errorf("failed to create %s: (%d) %s %+v", resourceName, res.StatusCode, res.Status, string(b))
	}
...
func (c *Client) UpdateClient(specClient *v1alpha1.KeycloakAPIClient, realmName string) error {
	// Pace Keycloak Workaround Fix
	if len(specClient.DefaultRoles) == 0 {
